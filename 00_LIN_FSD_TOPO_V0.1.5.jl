...
const sigma_all	= 3.0f0
const max_all_t = 5.0f0

const max_penalty = 5
const thick_ini = 1.0f0		
...
F = zeros(Float32,  2*(nely+1)*(nelx+1))	# Initialize external forces vector
# Loads
F[2] = -1.0f0	   # Set applied external force	
	
fixeddofs = [(1:2:2*(nely+1));  2*(nely+1)*(nelx+1) ]  # Set boundary conditions
	
U = zeros(Float32,  2*(nely+1)*(nelx+1))	# Initialize global displacements
...
iK = kron(edofMat,ones(Int32,8,1))'[:]
jK = kron(edofMat,ones(Int32,1,8))'[:]
	
# Initialize iterated thickness in domain
t_iter = ones(Float32,1:nely,1:nelx).*thick_ini  	
t_res = []	# Array of arrays with iteration history of thickness	
	
canvas = OffsetArray(zeros(Float32, 1:nely+2, 1:nelx+2), 0:nely+1,0:nelx+1)	
	
S = zeros(Float32,1:nely,1:nelx)  # Initialize matrix containing field results (typically a stress component or function)	
...
Gauss_3x3_kernel = @SMatrix [1.0f0 2.0f0 1.0f0 ;
				    		 2.0f0 4.0f0 2.0f0 ;
				    		 1.0f0 2.0f0 1.0f0 ] 	
...
# Principal stresses
s1 = 0.5f0 * (sxx + syy + √((sxx - syy) ^ 2 + 4f0 * sxy ^ 2))
s2 = 0.5f0 * (sxx + syy - √((sxx - syy) ^ 2 + 4f0 * sxy ^ 2))
S[y, x] = √(s1 ^ 2 + s2 ^ 2 - 2 * 0.3f0 * s1 * s2)  # elastic strain energy??
...
for gauss in 1:max(0,(ceil(scale*(iter-.8*Niter)/(2*-.8*Niter)))) 
