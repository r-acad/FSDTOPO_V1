...
heatmap(reverse(S .* t_iter ./5  , dims = 1), clim = (4.5, 16), aspect_ratio = 1)
...
# ╔═╡ b0e0f3cd-1a5f-4d4c-a230-8833568c1265
begin

# Initialize iterated thickness in domain
#t_iter = (view(canvas,1:nely,1:nelx)).* max_all_t 
...
#S[y, x] = √(s1 ^ 2 + s2 ^ 2 - 2 * 0.3 * s1 * s2)  # elastic strain energy?? ORIG

S[y, x] = √(s1 ^ 2 + s2 ^ 2 + .6* s1 * s2)  # elastic strain energy??				

#S[y, x] = √(s1 ^ 2 + s2 ^ 2 )  # elastic strain energy??					
...
push!(t_res, copy(S .* t_iter ./max_all_t))			
...
function plot_animation()
	...
	gif(anim_evolution, "latest_LIN_topo.gif", fps = 6)
...
heatmap(reverse(t_iter, dims=1), aspect_ratio = 1, c=cgrad(:jet1, 10, categorical = true))
end	
