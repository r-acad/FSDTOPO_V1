...
begin
	...
	const Niter_ODE = 1001 # Number of iterations in solver
		
	initial_mass =   5. #*  explicit_scale  # Initial atom mass
	
	const mu =  .7 #* explicit_scale # Initial atom damping coefficient	
	const fr =  0.05  # Friction coefficient in Verlet with friction
	const G = 9.81 * 1.0
	
	const Δt = .09   # Time step
...
a_F[2,natoms_r,1,n] += - 2. #* amplitude 		
...
Klink = 20 *  1/ (1/a_M[i,j] + 1/a_M[i+offset[1],j+offset[2]] ) / offset[3]
