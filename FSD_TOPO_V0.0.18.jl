...
V0.0.18 Go to basic Verlet without forces dependant on velocities
...
begin
	...
	initial_mass =   5. #*  explicit_scale  # Initial atom mass
	
	const mu =  .7 #* explicit_scale # Initial atom damping coefficient	
	const fr =  0 #0.01  # Friction coefficient in Verlet with friction
	const G = 9.81 * 1.0
	
	const Δt = .05   # Time step
...
a_M = OffsetArray(zeros(Float64,natoms_r+2,natoms_c+2),
	              0:natoms_r+1, 0:natoms_c+1)  # Array of atom "masses" (makes Klink as product of masses divided by rest-length) at current topo iteration	
...
# Apply external forces	in lattice
...
a_F[2,natoms_r,1,n] += - 200.# * amplitude 		
		

# Traverse complete lattice		
@inbounds  for i = 1:natoms_r, j = 1:natoms_c, dim = 1:ndims 
...
#Klink = 200 *  1/ (1/a_M[i,j] + 1/a_M[i+offset[1],j+offset[2]] ) / offset[3]
Klink = 100 *  (a_M[i,j]) / offset[3]
...
a_x[dim, i,j, n+1] = (2-fr) * a_x[dim, i,j, n] - (1-fr)* a_x[dim, i,j, n-1] + a_F[dim, i,j, n] * Δt^2 / a_M[i,j] 				
